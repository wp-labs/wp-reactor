...
/// 放专用线程反而避免 executor 饥饿）。worker 数固定 1（FIFO 保序）。
pub struct AsyncPersister<T, B> {
    tx: Option<SyncSender<(Vec<T>, usize)>>,
    /// 队列字节预算（背压阈值）。
    byte_budget: usize,
    /// 当前在途字节（已提交未写清）。
    queued_bytes: Arc<AtomicUsize>,
    /// 队列空标志（Condvar 通知；worker 写清后置 true）。
...
impl<T: Send + 'static, B: BatchWriter<T> + Send + 'static> AsyncPersister<T, B> {
    /// 创建并启动后台 worker。
    ///
    /// - `backend`：写后端（worker 线程持有）。
    /// - `byte_budget`：队列字节预算（超限 [`PersistError::Backpressure`]）。
    /// - `max_pending_batches`：通道批容量（防批数无限堆积）。
                ...
                loop {
                    ...
                    if let Err(e) = backend.write_batch(items) {
                        (error_cb_w
                            .as_ref()
                            .map(|cb| cb(&e))
                            .unwrap_or_else(|| eprintln!("[async-persist] 写失败(丢弃该批): {e}")));
                    }
                    ...
                    // 无条件 notify：字节预算等待方（submit 背压）依赖它醒后重查。
                    cvar.notify_all();
                }
            })
    ...
    pub fn submit_batch(&self, items: Vec<T>, est_bytes: usize) -> Result<(), PersistError> {
        ...
        // 通道满 → 阻塞（背压）；通道关 → Closed。
        match tx.send((items, est_bytes)) {
            Ok(()) => {
...
mod tests {
    ...
    fn submit_flush_writes_all_in_order() {
        ...
        let p = AsyncPersister::<u64, SharedBackend>::new(backend.clone(), 1 << 20, 16, None);
        for i in 0..100u64 {
            p.submit_batch(vec![i], est(1)).unwrap();
        }
        p.flush().unwrap();
    ...
    fn byte_budget_backpressure_blocks_not_drops() {
        ...
        let p = AsyncPersister::<u64, SharedBackend>::new(backend.clone(), 16, 4, None);
        p.submit_batch(vec![1], est(1)).unwrap();
        p.submit_batch(vec![2], est(1)).unwrap();
        p.submit_batch(vec![3], est(1)).unwrap(); // 阻塞直到预算可容纳
        p.flush().unwrap();
    ...
    fn error_cb_invoked_on_write_failure() {
        ...
        let p =
            AsyncPersister::<u64, SharedBackend>::new(backend.clone(), 1 << 20, 8, Some(err_cb));
