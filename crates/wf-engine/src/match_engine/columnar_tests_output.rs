...
fn normalize_int_to_number(v: Value) -> Value {
    match v {
        ...
        Value::Object(fields) => Value::Object(
            fields
                .into_iter()
                .map(|(k, v)| (k, normalize_int_to_number(v)))
                .collect(),
        ),
        other => other,
    }
...
fn assert_output_equiv(expr: &Expr, batch: &RecordBatch) {
    ...
    for (row, event) in events.iter().enumerate() {
        ...
        assert_eq!(
            normalize_int_to_number(columnar),
            normalize_int_to_number(interpreted),
            "row {row}: expr={expr:?}"
        );
    }
}
...
fn assert_value_equiv(expr: &Expr, batch: &RecordBatch) {
    ...
    for (row, event) in events.iter().enumerate() {
        let columnar = cvec
            .scalar_at(row)
            .map(|s| normalize_int_to_number(cscalar_to_value(&s)));
        let interpreted = eval_expr(expr, event).map(normalize_int_to_number);
