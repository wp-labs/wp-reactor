//! close_bench 拆出的兄弟子模块（2026-09-04）：Q15 close 累积路径逐分量微基准
//! ——fixed + `and close` 规则每事件 `accumulate_close_steps` 的 ns/事件归因
//! （baseline / guards / distinct_valkey / distinct_i64 / collect_clone /
//! amort_slice_10s，背景与数据版说明见父模块 doc），及状态机 advance 之外的
//! 生产外围每行循环对照（masks_build / scan_per_row / deferred_row / eager_row /
//! prod_row_full）。共享 harness/import 在父模块 close_bench.rs，此处经
//! `use super::*` 复用；切片内独占构造（v_f64 / q15_rule_plan / q15_batch /
//! q15_production_path）随迁。
