...
use crate::match_engine::{
    extract_key_simple, extract_scope_key_mixed, field_ref_name, scope_key_from_values,
...
fn expr_partition_rows_equals_precomputed_field_column() {
    ...
    let field_per = partition_rows_by_key(&pair_batch, &[FieldRef::Simple("pair".into())], shards)
...
fn expr_numeric_partition_matches_precomputed_int_column() {
    ...
    let field_per = partition_rows_by_key(&sum_batch, &[FieldRef::Simple("sum".into())], shards)
...
fn expr_mixed_key_partition_matches_row_based() {
    ...
    for (row, ev) in events.iter().enumerate() {
        let col_s = per
            ...
            .position(|rows| rows.contains(&(row as u32)))
            .unwrap();
        let row_s = extract_scope_key_mixed(ev, spec.keys.as_ref(), spec.key_exprs.as_ref(), "")
