...
fn conv_top_ties_keeps_rank_1_ties() {
    ...
    let result = crate::cep::apply_conv(&plan, &[FieldRef::Simple("sip".into())], outputs);
...
fn conv_sort_descending() {
    ...
    let result = crate::cep::apply_conv(&plan, &keys, outputs);
...
fn conv_empty_outputs_noop() {
    ...
    let keys = vec![FieldRef::Simple("sip".into())];
    let result = crate::cep::apply_conv(&plan, &keys, vec![]);
...
fn conv_top_n_must_aggregate_across_shards() {
    ...
    let per_shard = crate::cep::apply_conv(&plan, &keys, shard_b.clone());
    ...
    let aggregated = crate::cep::apply_conv(&plan, &keys, merged);
