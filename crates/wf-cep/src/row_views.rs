//! 列式行 / 触发视图（P4-B0/B1 下沉，2026-09-04）：`ColumnarEvent`（on-each
//! 完全不物化）、`JoinRow`（join 候选行）与 `TriggerEvent`（P4 触发行）都按需从
//! Arc 批读字段（零 HashMap 物化）；`column_scalar_string` 供 `window.has()`
//! 单列建 set。值转换走 `crate::value_extract`（与行式 Event 路径字节一致，
//! 对拍锁定）。引擎经 event_bridge / window 的 shim 重导出，公开路径不变。
...
impl<'a> ColumnarEvent<'a> {
    ...
    /// The row index this view reads within the shared batch (2026-09-04
    /// P4-B1 起跨 crate 被 engine 列式 executor 消费)。
