...
"#,
    ...
    let (schemas, configs) = build_pipeline_internal_windows(&plans, &base_schemas, &defaults());
...
"#,
    ...
    let (schemas, _configs) = build_pipeline_internal_windows(&plans, &base_schemas, &defaults());
    assert_eq!(schemas.len(), 1);
    let ws = &schemas[0];
    assert_eq!(ws.name, "__wf_pipe_pipe_w1");
    assert_eq!(
