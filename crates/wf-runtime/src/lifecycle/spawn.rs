...
#[path = "spawn_receiver.rs"]
mod receiver;

pub(super) use receiver::spawn_receiver_task;
#[path = "spawn_metrics.rs"]
mod metrics;

pub(super) use metrics::{metrics_record_to_data_record, spawn_metrics_task};

// 仅测试（spawn_coverage* / mod tests）消费的 receiver 组 helper——lib 构建无生产
// 使用，`#[allow(unused_imports)]` 组合（StatsBucket / accumulate_* 先例）。
#[allow(unused_imports)]
use receiver::{
    register_builtin_external_sources, resolve_connector_kind, resolve_source_path,
    source_data_format, source_param_to_json, source_stream_tag,
};
