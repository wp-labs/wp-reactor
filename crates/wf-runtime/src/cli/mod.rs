...
async fn run_cli_inner() -> EngineResult<()> {
    ...
    match cli.command {
        Commands::Run {
            ...
            metrics_interval,
            metrics_listen,
        } => run_engine(load, metrics, metrics_interval, metrics_listen).await,
        Commands::Config { command } => run_config_command(command),
    }
}

/// `wfusion run` —— 加载配置 → 启动 Reactor 并驱动到退出。
async fn run_engine(
    load: ConfigLoadArgs,
    metrics: bool,
    metrics_interval: Option<String>,
...
/// `wfusion config` 子命令分派。
fn run_config_command(command: ConfigCommands) -> EngineResult<()> {
    match command {
        ConfigCommands::Render { load, raw } => render_config(load, raw),
        ConfigCommands::Origins { load, filter } => show_config_origins(load, filter),
        ConfigCommands::Vars { load, filter } => show_config_vars(load, filter),
        ConfigCommands::Diff {
            load,
            compare,
            filter,
            expanded,
        } => diff_config(load, compare, filter, expanded),
    }
}

fn render_config(load: ConfigLoadArgs, raw: bool) -> EngineResult<()> {
    let resolved = resolve_config_load(load)?;
    let loader = FusionConfigLoader::new(
        &resolved.config_path,
        &resolved.overlay_paths,
...
fn show_config_origins(load: ConfigLoadArgs, filter: PathFilterArgs) -> EngineResult<()> {
    let resolved = resolve_config_load(load)?;
...
fn show_config_vars(load: ConfigLoadArgs, filter: VarFilterArgs) -> EngineResult<()> {
    let resolved = resolve_config_load(load)?;
