...
impl RuleTask {
    ...
    #[allow(clippy::too_many_arguments)]
    async fn process_batch_machine_rows(
        &mut self,
    ...
    /// 各自持 &mut self 无冲突。
    #[allow(clippy::too_many_arguments)]
    async fn process_batch_each_rows<'rows>(
        &mut self,
        alias: &str,
        window_name: &str,
        batch_seq: u64,
        ...
        for i in 0..row_domain.len() {
            let row_index = row_domain.row_at(i);
            let event: Option<&Arc<Event>> =
                eager_events.as_ref().map(|events| &events[row_index]);
...
fn scan_expired_and_route_closes(
...
#[allow(clippy::too_many_arguments)]
fn advance_machine_row_aliases(
    machine: &mut CepStateMachine,
    executor: &RuleExecutor,
