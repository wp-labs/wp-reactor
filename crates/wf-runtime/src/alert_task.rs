...
#[jumo(kind = "state", domain = "Runtime", module = "Runtime.AlertDispatch")]
...
#[jumo(kind = "struct", domain = "Runtime", module = "Runtime.AlertDispatch")]
...
async fn dispatch_batch(
    ...
    // perf-diag cut_sink_write 门控：AlertBatch 到 sink 即丢（不物化/序列化/写）——
    // 测「输出构建 + 通道投递」段; 增量（full − emit）= 列→行物化 + 序列化 + sink 写成本。
    // 哨兵批（yield target = `__wf_sentinel`）豁免——测量协议（档位切换 + EPS）
    // 必须落盘。旧判定按 sink 名（`sentinel_out` ≠ `__wf_sentinel`）不匹配：
    // 7 档墙梯（emit 档 cut_sink_write=true）下哨兵记录被切 → wfgen 等哨兵超时
    // → 墙梯假死（2026-08-26 定位修复：改按 batch target 判定）。
    if crate::perf_diag::perf_cut_sink_write() && !is_sentinel_batch(&batch) {
        return;
    }
...
mod tests {
    ...
    fn normal_columns_batch_is_not_exempt() {
        // 普通业务批（如 q19 top 输出）在 cut_sink_write 下必须被切。
