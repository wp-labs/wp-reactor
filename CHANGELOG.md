...
- 新增嵌套层次硬限制用例：表达式嵌套 5 层通过 / 6 层报错（顶层与阈值入口同口径）、200 层括号快速失败不再栈溢出；`let` 链 5 层通过 / 6 层与成环报错、200 层链作为 key 也必须 fail-closed 不崩；内联器链深边界。`wf-cep` 369 / `wf-engine` 1053 / `wf-lang` 1219 / `wf-runtime` 639 全绿。
