...
- **规则级 `let` 前向引用改为明确报错（warp-fusion#101 审查）**：`let a = b` 而 `b` 声明在后时，此前会以“字段 `b` 在任何事件源中都不存在”报错——错误方向指向 window schema，容易把用户带偏；现直接提示“`a` 引用了尚未声明的 `let` `b`（规则级 `let` 按声明顺序解析）”。**仅改错误文案**：判定结果与可接受的规则集完全不变（名字同时是该 window 的字段时照旧解析为字段）。
- **新增嵌套层次硬限制（防栈溢出崩溃，5 层）**：递归下降解析与多处按结构递归的展开（`let` 引用链：键派生 / 列式 yield / 阈值常量内联）此前对用户输入深度**没有任何上限**——一段约 200 层括号的规则（`min >= ((((…1…))))`）或约 200 层 `let` 链会让编译进程**栈溢出直接 abort**（栈溢出不是 panic、catch 不住），且没有位置信息：编辑器语言服务进程崩溃、规则加载失败、CI 只报 `signal: 6`。现统一为编译期硬限制：**表达式嵌套分组 ≤ 5 层**（括号 / 函数实参 / 数组·对象元素 / `in (...)` / `if`·`case` 分支各算一层，顶层不计）与**规则级 `let` 引用链 ≤ 5 层**（成环一并报错），超限给带位置的解析 / 语义错误。
- **算子链同样设硬上限（同一类崩溃的第二条入口）**：`a + b + c`、`a || b || c`、`not not …` 在解析器里是循环、不吃解析栈，却会构造出与项数同阶的**左深表达式树**，下游按结构递归的遍历（类型检查 / 常量折叠 / `let` 内联 / 键派生 / 代码生成）照样把栈打爆——实测 `yield` 中 5000 项的加法链、以及「3 层分组 × 每层 128 项链」都会 `abort`。现限制**同一分组内的算子链 ≤ 16 层**（分组重置计数 ⇒ 长 `in (...)` 列表与长实参列表不受影响）；与嵌套分组合计，任一表达式沿路径的链式深度 ≤ 96 层。需要更长的表达式时改用 `in (...)` 列表或规则级 `let`（既有语料实测最长算子链 6 项，不受影响）。
...
- 新增 issue #101 阈值用例：编译期拒绝（L3 函数、字段引用、非常量 `let` 引用、`now*` / `baseline`、`if…then…else`、除零 / 模零，含 pipeline stage 与 `on event seq` 位置、错误计数与表达式回显）、常量正例（字面量、取负、括号常量算术、字符串字面量、规则级常量 `let`），以及常量 `let` 内联的 plan 断言、“常量 `let` 阈值真实触发”与“内联版≡手写字面量”差异等价端到端、内联器自身单测（取值/链/自引用与互引用/深链/退化常量/遍历）、常量 `let` 在 close 与 seq 与 `on event<accu>` 位置可用、pipeline stage 与重名 `let` 拒绝、lint W004 穿透常量 `let`、语言侧↔运行期折叠规则一致性守卫与共享折叠单测。
- issue #101 审查补充用例（跨特性交互）：`||` 多分支各自引用不同常量 `let` 时**两侧都**内联为字面量、任一分支引用非常量 `let` 时该分支被拒；布尔常量 `let` 作阈值归为类型错误（而非“不可求值”）；`let` 与字段同名不改变非阈值位置（`entity` 键）的解析；`let` 声明在 `match` 之后为语法错误；前向引用 `let` 给出声明顺序错误且不残留字段缺失噪音，同时锁定「名字兼作字段时仍解析为字段」这一既有语义不被误改（零行为变化的守卫）。另补“内联版 ≡ 手写字面量版”的 `explain` 逐字一致与字段用量无幽灵列断言，并对每处修复做过变异验证（逐项回退后对应用例必红）。
- 新增表达式链深用例：预算内的加法链 / `||` / `&&` / `*` / `not` 前缀通过、超限给可读错误、5000 与 20000 项链快速失败不再栈溢出；长 `in (...)` 列表、长数组、长函数实参列表不受影响（扁平列表不吃预算）；分组重置计数与兄弟不累加；并锁定「最深的合法形状」（5 层分组各带用满预算的左深链，路径深度约 90）能走完解析 / 语义检查 / 编译——保证上限不是宽松到又能打爆下游。
- 新增嵌套层次硬限制用例：表达式嵌套 5 层通过 / 6 层报错（顶层与阈值入口同口径）、200 层括号快速失败不再栈溢出；`let` 链 5 层通过 / 6 层与成环报错、200 层链作为 key 也必须 fail-closed 不崩；内联器链深边界。`wf-cep` 369 / `wf-engine` 1053 / `wf-lang` 1240 / `wf-runtime` 639 全绿。
