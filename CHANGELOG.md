...
- **新增嵌套层次硬限制（防栈溢出崩溃，5 层）**：递归下降解析与多处按结构递归的展开（`let` 引用链：键派生 / 列式 yield / 阈值常量内联）此前对用户输入深度**没有任何上限**——一段约 200 层括号的规则（`min >= ((((…1…))))`）或约 200 层 `let` 链会让编译进程**栈溢出直接 abort**（栈溢出不是 panic、catch 不住），且没有位置信息：编辑器语言服务进程崩溃、规则加载失败、CI 只报 `signal: 6`。现统一为编译期硬限制（同一常量）：**表达式嵌套分组 ≤ 5 层**（括号 / 函数实参 / 数组·对象元素 / `in (...)` / `if`·`case` 分支各算一层，同一层算子链不递增、顶层不计）与**规则级 `let` 引用链 ≤ 5 层**（成环一并报错），超限给带位置的解析/语义错误。既有规则语料与全部测试在 5 层下不受影响。
...
- 新增嵌套层次硬限制用例：表达式嵌套 5 层通过 / 6 层报错（顶层与阈值入口同口径）、200 层括号快速失败不再栈溢出；`let` 链 5 层通过 / 6 层与成环报错、200 层链作为 key 也必须 fail-closed 不崩；内联器链深边界。`wf-cep` 369 / `wf-engine` 1053 / `wf-lang` 1228 / `wf-runtime` 639 全绿。
