...
- 新增 issue #101 阈值常量性用例：编译期拒绝（L3 函数、字段引用、规则级 `let` 引用、`now*` / `baseline`、`if…then…else`、除零 / 模零，含 pipeline stage 与 `on event seq` 位置、错误计数与表达式回显）与常量正例（字面量、取负、括号常量算术、字符串字面量）；另加语言侧↔运行期折叠规则一致性守卫与共享折叠单测。`wf-cep` 369 / `wf-engine` 1051 / `wf-lang` 1196 / `wf-runtime` 639 全绿。
