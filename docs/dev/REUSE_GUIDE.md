...
## 1. 适用范围（不只问题排查——日常开发主动使用）

**定位**：这不是一本"故障排查手册"——复用机制是**开发时的默认选择**，不是等
性能/内存出问题才回头翻：

**写代码时（主动复用，不等出问题）**
- 写任何 alert/输出/列式/emit 代码 → **§3.1 常量列折叠**、**§3.2 快车道**
  （全列同值的列、可直读的列，默认优先折叠/直写，别按行存冗余）
- 任何涉及 join/provider/静态表 → **§3.3 预物化**、**§3.4 批级预查**
- 任何采样/测量/报告（bench/diag/新指标） → **§4 口径**（RSS 用内部峰值、
  dirty 用 footprint、判据用 dirty）

**排查问题时（§2 定位工具链）**
- 查询 EPS 低 / RSS 超预期 / 内存随数据量增长 → **§2** 先定位段，再动手
- 定位到段后要优化 → **§3** 找现成可复用代码
- 要采内存/性能数据 → **§4** 用对口径，别被假象骗

**评审/交接时**
- 新改动是否遵循 §3 模式（该折叠的列没折叠 = 评审疑问点）
- 改动后是否按 §5 流程验证（bench 对比 + 守护测试）
