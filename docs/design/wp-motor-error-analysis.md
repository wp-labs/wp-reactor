...
1. **DomainReason 集中在独立 crate (wp-error)**: wp-reactor 的 crate 间依赖更简单（现行六 crate：wf-lang / wf-config / wf-cep / wf-data / wf-engine / wf-runtime），每个 crate 定义自己的 reason 再 err_conv 更清晰。
