...
1. **第一道墙:preread 预算槽位**(配置可解)。IPC 解码把 ~7.7MB wire 帧膨胀成
   ~116MB Arrow 内存(约 15×:Arrow padding/数组头),预算按解码内存计 →
   256MB 仅 ~2 槽 → 源被节流。`parse_buffer_bytes=4GB` → q1 +40%(5.94→8.31M)、
   q2 +23%。**关系为 Little's law(闭回路)`:吞吐 = 槽位/驻留,驻留随槽位增长
   (排队效应),故增益亚线性**——4GB 后已接近管线服务容量(第二道墙顶上),
   再加预算收益有限。
...
| **P0-②** | **解码内存膨胀修复**(预算按 wire/内容字节计,槽位翻 ~15×) | 二分法定位的第一道墙根因,真实负载最大普适杠杆 |
