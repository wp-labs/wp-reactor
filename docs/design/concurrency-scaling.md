...
## 两道墙模型(二分法实证,2026-08-17 修订)

> **修订(P0-②,2026-08-17)**:第一道墙的“解码内存膨胀 ~15×”是记账假象——
> IPC 解码后 `get_array_memory_size` 对真实数据结构性高估 ~10×(实测 71B/行
> wire → 718B/行 记账,与字段宽度无关),真实在途 footprint 是 wire 级。记账
> 单位改为 `content_bytes`(≈ wire)后,第一道墙基本消融,剩一个**管线深度
> 节流语义**:

1. **preread 预算(记账修正后成为“深度节流器”)**。解码记账下 256MB 仅 ~2 槽
   → 源被节流(`parse_buffer_bytes=4GB` → q1 +40%、q2 +23%)。P0-② 改为按
   `content_bytes` 记账后,同样预算槽位 ×~10:256MB ≈ 36 槽、**2GB ≈ 288 槽
   为甜点**(q1 7.58M / q2 7.23M,旧 4GB 解码记账口径 q1 7.19M / q2 7.19M);
   预算继续放大(4GB content,~556 槽)反而退化到 ~5.9M——**预算必须有界**:
   它不但是内存阀,也是管线深度节流器,过深缓冲放大窗口重排(见下);
   关系为 Little's law(闭回路):吞吐 = 槽位/驻留,驻留随槽位增长(排队效应),
   故有峰值后衰减(实验记录见 preread-budget-design.md §6)。
2. **第二道墙:窗口 actor 单写者串行**(结构性)。每窗口一个 actor:重排(等
   (source,seq) 乱序到齐)+ append(BTreeMap)+ 广播 + ack;窗口 mailbox 预算
   按 content 记账(64MB / 批 ~7.7MB ≈ 8 槽),dispatch 串行 → 反压回 preread
   预算。**预算加深会放大这道墙**:管线缓冲越深,actor 重排待补集越大、
   (source,seq) 缺口等待气泡越多(实验⑤ 放大 mailbox 1GB 反而 6.6M;P0-② 4GB
   content 同样退化 5.9M 即同一机制)。对无状态 on-each(q1)窗口是纯开销(可
   旁路,但真实负载覆盖率低);对有状态规则是语义核心(保序/驱逐/记账),只能
   实现层优化;key 分片(W-WCP)只对高 fanout 窗口读有效。
...
- **基准参考(q1/q2 100M 稳定口径,2026-08-17)**:P0-②(content 记账)后
  bench 默认 `parse_buffer_bytes=2GB`(CONNECTIONS=4 / SHARD_KEYS=bid_events:auction
  / instances=4 / p=10 r=10 / 100k 帧)——**q1 = 7.56~7.58M、RSS ≈ 14.2~14.3GB**;
  **q2(stateful)= 7.23M、RSS ≈ 7.6GB**。对照旧解码记账口径:256MB → q1 5.93M
  / RSS 4.4GB;4GB → q1 6.71~7.19M / RSS 13.5~14.8GB、q2 7.19M / RSS 14.8GB。
  **净增益:同吞吐下配置预算降 2×、q2 RSS 减半;吞吐平或略升**;
- **默认值决策(2026-08-17 修正):`parse_buffer_bytes` 默认 256MB → 128MB**。
  实测 content 记账下同配置值 256MB(36 槽)的 RSS 从旧 4.4GB 涨到 12~14.5GB
  (吞吐 5.93→6.25~6.66M)——推翻 8-16 记忆“100M 6.8GB”的对外口径。而 **128MB
  (18 槽)实测 q1 6.13M / RSS 5.88GB**,在旧默认之上小幅提升(RSS 只多 ~1.5GB)、
  且避开了 256MB 的 12-14GB 平台——定为新默认;要更高吞吐显式调大(256MB ≈
  6.3~6.7M / RSS 12-14GB、512MB ≈ 7.0M、1-2GB ≈ 7.5M+,见
  preread-budget-design.md §6 曲线)。
- 待办(引擎级,非基准参数):规则输出列**批量构建**(commit_each_row 逐行
  Arc 分配/格式化,占规则计算 ~57%)——**只省 CPU,非吞吐杠杆**(二分法 ① 已证,
  见 §2.3);吞吐下一个突破点 = 第二道墙(窗口 actor 重排/单写者,P0-③)。
- **探针态说明(已解除)**:§2.3 二分法数字为输出/规则硬切的探针态
  (TEMP-PROBE,当时工作区未提交);随后探针已全部还原(git checkout 干净),
  正常形态(输出开启)的基准见上——P0-② 的 1GB/2GB/4GB 曲线即正常形态测得。
...
| 优先级 | 项 | 状态 |
|---|---|---|
| ~~P0-①~~ | ~~`parse_buffer_bytes` 放大~~ | **并入 P0-②**——记账单位修正后甜点移回小预算(2GB content),放大不再是杠杆 |
| **P0-②** | **解码内存膨胀修复**(预算按 content/wire 字节计) | **已实施(2026-08-17)**:`push_decoded_batch`/IPC replay 改收 `content_bytes`。实测甜点 1~2GB content:q1 7.58M / q2 7.23M,与旧 4GB 解码记账持平且 q2 RSS 减半;4GB 过度缓冲退化(§2.3 两道墙、preread-budget-design.md §6) |
...
7. **2026-08-17(P0-② 实施)**:记账单位 `get_array_memory_size` → `content_bytes`
   (实测解码高估 ~10×,真实在途为 wire 级)。曲线:256MB→6.25~6.66M / 512MB→
   7.02M / **1GB→7.56M** / **2GB→7.58M(q1)、7.23M(q2,RSS 7.6GB)** / 4GB→5.90M
   (过度缓冲退化)。bench 默认转正 2GB content;默认配置 256MB 不变(≈36 槽)。
8. **2026-08-17(默认值修正,两轮)**:实测 content 记账下 256MB 的 RSS 涨到
   12~14.5GB(旧 4.4GB)——推翻了 8-16 的“100M 6.8GB”口径。先试 64MB(9 槽,
   5.90M/4.45GB,与旧默认一致),再测 **128MB(18 槽,6.13M/5.88GB):小幅吞吐
   提升 + RSS 只多 1.5GB**,定为新默认——默认行为在“完全不破口径”与“吃
   吞吐红利”之间取平衡;吞吐优先者显式调大(512MB-2GB)。
