...
| 优先级 | 项 | 状态 |
|---|---|---|
| **P0-①** | C-UCP 连接数 + W-RDP `instances` | 稳态下非杠杆(c4/instances=4 即够,双层墙在引擎内) |
| P0-② | 规则输出列**批量构建**(commit_each_row 逐行 Arc/格式化,占规则计算 ~57%) | 待办(引擎级) |
| P0-③ | 生产侧解码/驻留优化(外部 crate 解码计时未测) | 待查 |
| P1 | W-PDP / W-EDP / W-SDP 在双层墙打破后重测 | 待测 |
...
5. 2026-08-17(晚):管线埋点定位 q1 双层墙(生产侧 ~6.5M + 规则侧 ~6.1M),
   排除输出/批大小/预算/并行度;稳定基准 q1 5.93M / RSS 4.4GB / CPU ~7.2 核。
