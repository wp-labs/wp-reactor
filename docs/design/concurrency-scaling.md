...
|---|---|---|---|---|---|
| **数据上报并发** | **C-UCP** | 采集侧 | 连接数(= 客户端进程数,send-arrow 每进程 1 连接) | 连接 8→16 +50%、16→32 +12.5%,**未饱和**(纯 copy 供给口径†) | **主杠杆**(仅纯 copy 口径†) |
| 引擎读取并行 | **W-RDP** | 引擎 | `instances`(wp-core-connectors 已有,1..=16) | 1:1 曲线 4.2→9.9M(纯 copy 口径†);8 实例+16 连接 ≈ 16 实例+16 连接 | **零代码**;最优 ≈ 连接数/2 ~ 连接数(纯 copy 口径†) |
...
2. **C-UCP 是主变量且未饱和(纯 copy 供给口径†)**:连接 2→4→8→16(W-RDP=8 固定):
   4.9→5.3→6.8→10.2M,单调;16→32 仍 +12.5%。唯一数据稳态下连接数非杠杆(§3.1)。
...
5. **高并行下资源余量大(纯 copy 供给口径†)**:C-UCP=32/W-RDP=16 时 EPS 11.5M,CPU 仅
   ~3 核(81% 线程等待),RSS ~0.75GB——并行档位远未用满。**†口径说明**:纯 copy 供给 =
   每条连接推**完整帧文件**(重复数据),append 到阈值即 kill 客户端,测的是爬坡瞬态;
   RSS 0.75GB 正是窗口未积累的特征。**唯一数据稳态不随连接数涨**(§3.1:q1 5.93M,
   RSS 4.4GB,CPU ~7.2 核,双层墙)。
...
> **口径声明(2026-08-17 复盘)**:本节及 §1 的 4.2→11.15M 数据均为**纯 copy 供给口径**
> ——每条连接推完整帧文件(重复数据),`engine_appended` 到阈值即 kill,测的是爬坡
> 瞬态(RSS ~0.75GB = 窗口未积累)。**唯一数据稳态(分片文件注入)不随连接数线性涨**:
> c2/4/16 = 5.56/5.92/5.93M,双层墙 ~5.9-6.5M,详见 §3.1。下表是供给能力的上限探针,
> 不是稳态吞吐。

排除清单(全部无效):parse 并行度 10→20、预算 ×4~16、去 alert 输出(CPU 骤降 8→2 核,
EPS 不变)、decode(profile 0.3%)、staging 256→512KB、客户端单连接能力(7.75M/s)、日志。
(2026-08-17 晚分片文件复核一致:预算 256MB→1GB 仅 +3.4%,p/r 10→20 无效,屏蔽输出仅 +7%。)
...
**当前形态(纯 copy 供给口径)**:C-UCP=32 / W-RDP=16 / W-WCP=1 / W-PDP=10 / W-EDP=10 /
W-SDP=8,EPS 11.5M,CPU 3 核,RSS 0.75GB(爬坡瞬态,非稳态)。**稳态(唯一数据)上限
见 §3.1:q1 5.93M / RSS 4.4GB / CPU ~7.2 核,双层墙(生产侧 ~6.5M + 规则侧 ~6.1M)**。
"大帧批量化"已实测无效(25k/100k/400k 均无影响,§3.1 排除实验)。
