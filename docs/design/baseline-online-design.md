...
> **谁提供历史、引擎如何读到、相位如何推进**。本补充把评审出的候选通道收敛为
> **两条判定通道**（近端 B / 远端 A），并确立数据契约（任何通道/实现都必须对齐）。
>
...
metrics 事件 ─▶ baseline_producer（stats 窗 + sumsq）
                 └─▶ yield ─▶ sink ─▶ t_baseline（PG/Doris/本地 redb，唯一事实源）
                    ...
                    ┌───────────────────┴───────────────────┐
                    ▼                                       ▼
        近端 B：规则级基线历史表              远端 A：全历史相位桶三元组
        （逐窗/桶态三元组，收盘增量             （SQL 视图/供给 → knowdb
         append + 启动 warm，事件时间           ProviderWindow refresh → join，
         精确）                                处理时间近似）
                    └───────────────────┬───────────────────┘
                                        ▼
                  引擎内合并函数（§5 语义，唯一一份）
...
"远"（长留存/跨进程共享、处理时间近似）。二者同源、合并函数唯一、数据契约唯一。

> external 壳 C（低频服务化出口）**已排除，不做**（2026-09-08）：① 引擎内判定两档
> （B 热路径 / A 长程）已闭环，壳 C 无新语义；② 它面向"外部消费者查询当前偏离度"——
> 那是**宿主产品 API 层**职责（消费同一数据契约/供给即可），不在引擎内再造第三份
> 状态与合并逻辑；③ 无实际消费者，无驱动需求。见 §11.4。
...
| **S2-4** | 判定热路径 | 每事件判定走**近端 B**（≈内存读，不碰库/不碰 join）；A 仅作长程/冷/跨进程；判定出口 = B 告警 + A 供给，external 不承担 baseline 通道（见 11.4） | 高 EPS 不下网络、不阻塞 worker |
...
| ~~external 壳 C~~ | **❌ 已排除（2026-09-08）** | 引擎内两档已闭环无新语义；"外部查询当前偏离"属宿主产品 API 层（消费同一数据契约），引擎内另造 external 通道 = 第三份状态 + 重复合并逻辑，违背 §11.3 单一合并；无实际消费者 |
...
| ~~S2-M4~~ | ~~external 壳 C（低频服务化）~~ **不做**（2026-09-08 排除）：低频服务化出口无独立消费者，判定出口 = 近端 B 告警 + 远端 A 供给；外部查询式消费属宿主产品 API 层职责 | — |
