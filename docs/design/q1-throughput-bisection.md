...
> **当前权威现状见 §24（2026-08-19）**：100m 稳态主墙是 fill，L3 批式列写已无条件落地
> （commit `a5ca4d2`，EPS ~27-33M、EMIT 92M 精确）。
> **测量纪律（§7-§12 定案 + §24 修正）**：本机是**常载开发机**（loadavg 6.5-8.8+），
> 负载噪声使 A/B **必须同 load 块配对**；且**对比必须同 total（100m 稳态）**，30m 是瞬态
> 不可比（§24）。早期「双峰相位 ±8%」与「54M = 后台干扰」都是误判（§12 定案：
> 43/55/84M 三档是 bench.sh 计时伪影，真实 decode ≥84M）；同 load 块配对仍是必要纪律。
...
> ✅ **本节指出的主攻点已落地（2026-08-19，commit `a5ca4d2`）**：L3 批式列写
> （`commit_each_rows_batch`）已无条件成为 on-each 输出路径，Q1 100m 稳态
> EPS ~27-33M（同 load 配对，EMIT 92M 精确、`[clean]`）。
> **本节「cut C +167%」「收益上限 ~60%/2.7×」是 12.1M 基线口径的估计，随基线漂移**：
> 100m 稳态下 fill 切为 +3.4×（12 → 41M，见 §24）。历史数据保留作归因方法论。
...
> ✅ 第 1、2 条（L3 输出列式化 / wfx_id 列渲染）已落地：L3 批式列写
（`a5ca4d2`）
> 无条件化。以下结论是当时基线的归因，仍可作为「为什么 L3 值得做」的背景。
...
> ⚠ **本小节结论已被 100m 稳态推犻（2026-08-19）**：当时基于 30m 瞬态 / ~12M 基线得出
> 「fill 优化被并发稀释到 ~0」；100m 稳态（同 load 块配对）实测 fill 切为 +3.4×
> （12M → 41M），**fill 就是稳态主墙，并非被稀释**。30m 是瞬态不可比（53M 高估），
> 对比必须同 total（100m 稳态）——见文末 §24。以下保留当时探针数据仅作方法论参考。
...
### 17.5 并行度扫描：r=2/6/10（2026-08-18 晚，用户指定实验）

| r | EPS | 每任务 ns | 每任务相对 r=2 |
|---|---|---|---|
| 2 | 2.7M | 740 | 1.00× |
| 6 | 7.9M | 776 | 1.05× |
| 10 | 11.6M | 847 | 1.14× |

- **总吞吐 ≈ worker 数 × 每任务吞吐（近线性）**：每任务成本只随并行度
  +14% 爬升，远低于微基准 10 线程模拟的 3.3× 膨胀——端到端规则任务有
  通道接收/发送/批间空隙，实际 CPU 并发程度远低于「满负荷纯循环」。
- **降低并行度不能提升 EPS**（r=6 比 r=10 低 33%）——§17.3 的「并发膨胀
  主墙」结论修正：端到端每任务 ~740-850ns 与并行度弱相关，主墙仍是
  **execute 本身**（端到端 ~650-700ns/行，微基准 518ns 为下限参考）。
- r=10 为当前配置最优。

### 17.6 全链路并行度扫描：6:6:6 vs 8:10:10（用户指定实验）

instances（ingress.toml）+ parse + rule + CONNECTIONS 统一调 6：

| 配置 | EPS | cpu | 每规则任务 ns |
|---|---|---|---|
| instances=8, p=10, r=10, c=8 | **11.6-12.1M** | ~11 核 | 847 |
| instances=6, p=6, r=6, c=6 | 7.36-7.67M | ~7 核 | 781 |

- 每任务成本**没有**因 instances/parse 降而改善（781 vs 776-847ns）——
  进一步排除并发竞争假设：瓶颈是**每任务固定的 per-row 成本 ~780ns**
...
> ⚠ 本节末尾「wfx_id -88% 端到端不动 EPS、execute 已不是瓶颈」是 ~12M 基线的结论，
> 已被 §24 取代：L3 批写后 fill 被吸收、瓶颈形态已变。下面的语义变更本身仍有效。
...
> ⚠ 本节的「CPU 超订是端到端主约束」是 ~12M 基线下的判断，**已被 §24 取代**：
> 100m 稳态主墙是 fill（而非 CPU 超订），L3 批写落地后更不成立。保留作当时分析。
...
> ⚠ **本节是 08-18 晚的交接点，已被 §24（2026-08-19）取代**：当时「fill 是当前单线程
> 第一大项 / 端到端 12-13.5M / CPU 超订是主约束」在 L3 批写无条件化后不再成立。
> 权威现状见新增 §24。以下保留为当时交接存档。
...
## 23. EMIT 收尾采样伪影：Q1 correctness 读数偶发偏低（2026-08-19 记录）

**症状**：同 input 100m、同干净二进制，Q1 `emitted_total` 恒读数为 92M（= bid 数，
每 bid 一条），但偶发一轮读到 **81.65M**（-11.3%），一度被误判为「高负载丢 ~11% 行」
的正确性事故。重跑同框对照后确认为**收尾采样伪影，非丢数**：

| total | append bid | EMIT `q1_bid_passthrough`（多轮） | 判定 |
|---|---|---|---|
| 10m | 9.2M | **9,200,000**（精确） | 无损，输入=输出 |
| 30m | ~27.6M | 27,600,000（历史） | 无损 |
| 100m | 92M | 91,772,681 / 81,651,326 | 98.1% / 88.7%（波动） |

Q1 无 filter、无条件（`on each b -> score(1.0)`），10m/30m 证明 EMIT 恒等于 bid 数，
故真实 100m 应为 ~92M。读到的 81.65M 是 `report_result` 从 metrics.ndjson 聚合
`emitted_total` 时，exporter **1s 才落盘一次**、跑批结束瞬间最后一批 emit 尚未落盘
（或 daemon 已被 EXIT trap 清理）所致——与 §12.2 的 `append_total` T2 膨胀**同源**
（同一个 1s exporter 落盘滞后），只是作用在收尾的 `emitted_total` 上。

**为什么 Q2 没有这个问题**：Q2 命中率 ~0.8%、emit 量小（~74.7 万 vs Q1 的 9200 万），
收尾漏掉不足万级的零头在 74.7 万上不可见（100m 恒读 747816）；Q1 收尾若漏一整批
（几十万）在 9200 万里虽小，但对照 92M 期望时以百分比形式暴露（88.7%）。

**教训/口径**：
- **「EMIT 略低于输入×命中率」不等于丢数**——先与短跑（10m/30m，精确无损）对照，
  再做同输入多轮 100m 看波动带，别拿单轮偏低读数当正确性红警。
- 数据完整性以 `[clean]`（serialize_failed/dropped_late/cursor_gap/memory_evicted 全 0）
  为权威；Q1 的 81.65M 与 92M 都 `[clean]`，真实值 92M。
- 修复方向（同 §12.4 未做）：exporter 支持亚秒落盘，或 correctness 收尾改用引擎
  累计快照而非 1s 轮询聚合。

## 24. 权威现状（2026-08-19）：100m 稳态 fill 主墙 + L3 批写已落地

> 本节是当前（2026-08-19）Q1 on-each 吞吐的权威结论，取代上文所有按 30m 瞬态 /
> ~12M 基线得出的旧口径（尤其 §17.3 的「fill 被稀释到 ~0」、§18/§19 的「execute
> 已不是瓶颈 / CPU 超订是主墙」、§22 的交接状态）。

**测量纪律（决定性）**：本机常载（loadavg 波动大），**A/B 必须同 total（100m 稳态）
+ 同 load 块配对**。30m / 30M 是**瞬态**不可比绝对数——eg 30m p=10 读到 53M 是瞬态
高估，100m 稳态只有 ~12M；之前把 30m 当稳态是误判根源。

**100m 稳态主墙 = 填充（fill，输出列批化）**，L3 批写即为此落地：

| 配置（100m，同 load 块配对） | EPS | CPU | RSS | EMIT |
|---|---|---|---|---|
| 基线（fill 逐行） | 8-14M | ~990-1360% | 18-20GB | 92M 精确（`[clean]`） |
| fill 切（`WF_CUT_FILL=1`） | **~41M** | ~700% | 8.6GB | — |
| **L3 批写（已无条件化，`a5ca4d2`）** | **~27-33M** | ~720-1140% | ~9GB | **92M 精确（`[clean]`）** |

- **fill 切 12M → 41M（+3.4×）**：推翻了 §17.3「fill 优化被并发稀释到 ~0」——那些
  是 30m 瞬态 / 12M 基线时的错误归因；100m 稳态下 fill 就是主墙。
- **L3 批式列写（`commit_each_rows_batch`，commit `a5ca4d2`）已无条件成为 on-each 输出
  路径**：把逐行 10×`Vec::push` + `fill_row_gaps` 扫描改成段末一次列主序 bulk extend +
  块级填充；并修复了块级填充的**稀疏段中空缺错位**（fill 必须落 [real0,fill,real2]，
  不能补到块尾）。同 load 下较 fill 逐行基线 +305% EPS、CPU -16%、RSS 砍半。
- L3 相对 fill 切仍低（27-33M vs 41M）：fill 切是探针（不产出真数据），L3 是真实
  产出且已吸收 fill；差距来自仍存在的 yield stage / 收集开销。
- RXSS/CPU 同步以同 load 为准：L3 的 27M 出现在 load 8.8，fill 切 41M 在 load 低档；
  **不比同一 load 块的绝对 EPS 都是噪声**。

**当前已不成立 / 需以本节为准的旧结论**：
- §15「cut C +167%、收益上限 ~60%/2.7×」→ 基线与 L3 落地已变（见 §15 标注）。
- §17.3「fill 优化被并发膨胀稀释到 ~0」→ 100m 稳态 fill 主墙，被稀释不成立。
- §18「wfx_id -88% 不动 EPS，execute 已不是瓶颈」/ §19「CPU 超订是主墙」→ 当时基线
  （~12M）结论；L3 后 `process_batch` 内 fill 已被批写吸收，瓶颈形态已变。
- §22 交接状态 → 已陈旧，见本节。

**Q1 vs Q2 量级（当前）**：Q2（有 filter，只输出 ~0.81%）已 ~34-40M（ScopeKey 翻倍后），
Q1（无 filter 每 bid 全量输出，fill 主墙）~27-33M——Q1 反而比 Q2 慢，正因 Q1 的
fill 是每行代价；L3 批写已把 fill 主墙吸收。
