...
| 100M | 7.8M → **9.8-10.2M** | 20.4G → **3.8-4.9GB**（多次跑批） |
...
### 通用性验证（2026-08-26，100M 复测）
| Q | 形态 | 100M RSS（前 → 后） | 判据 |
|---|---|---|---|
| q5 | match+hop+字面量 yield | 17.5G → **7.9GB** | ✅ |
| q19 | stats+top-N | 32.9G → **14.4GB**（-56%） | ❌ 剩余为 stats 状态 |

- **q5 达标**：常量列折叠消除输出冗余后，窗口+在途主导（match 无重状态）。
- **q19 大降未达标**：内存墙显示窗口仅 2G（30M 口径），剩余 ~12G 是
  **stats 规则状态**（`top(10, price)` 每 auction 保留，键基数随 100M
  auction 数增长）——合法语义需要，非输出冗余（issue §6 预判：重状态类
  的非窗口内存可能是合法状态）。要再降需 stats 状态结构专项（非本优化
  适用范围）。
