...
阈值（`count >= 3` 里的 `3`）必须是**编译期常量**：数字 / 字符串字面量（可取负、可用括号做常量算术），或者引用规则级**常量** `let`（如 `let THRESHOLD = 3`——编译期内联为字面量后与手写等价）。字段引用、非常量 `let` 引用与函数调用不能用在阈值位置——它们在触发判定时求值不出结果，该分支会永不触发，因此编译期直接报错（issue #101）。
