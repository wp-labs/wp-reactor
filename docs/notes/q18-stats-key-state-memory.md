...
## 10.3 q18 决策（2026-08-26 结论）

- **q18 100M RSS 目标下调**：40G 中 9.8G 状态是语义必然（键数=数据特征），
  +3.5G 窗口有界——**硬性下界 ~13.5G，无法 <10G**。剩余 ~26G 为 close 期
  工作态，已拆解到无可再压的单点。
- **可选权衡**：WF_EMIT_CHUNK=200000 → 35G（-13%），EPS -9%——是否接受
  由用户定夺（未启用，默认仍 100 万链 = 40.5G）。

## 10.4 键数线性增长的根源（2026-08-26，用户质疑「并非每人对每场拍卖出价」）

**结论：2935 万键是权威数据生成器 + 权威语义共同决定，非实现 bug、非语义可减。**

- 权威语义（NEXMark q18.sql）：`ROW_NUMBER() OVER (PARTITION BY bidder, auction
  ORDER BY dateTime DESC) WHERE rank<=1`——分组键 `(bidder, auction)` 确认无误。
- **数据生成器（wfgen 忠实复刻官方 `nextBase0AuctionId`）**：bid 的 auction/bidder
  引用是「热批 + 冷随机滑动窗口」：
  - auction：50% 概率当前热批，50% 概率 `min_auc + rand(0..=110)`（最近 110 个
    in-flight auction 内均匀随机）；
  - bidder：75% 概率当前热批，25% 概率最近 1010 人内均匀随机。
  - **冷引用落在滑动窗口内任意点 → 随数据推进组合持续翻新 → (bidder, auction)
    组合键数随事件量线性增长**（30M 881 万 → 100M 2935 万，聚集度恒 0.319）。
  - 这是官方模拟真实时序的刻意设计（auction 窗口滚动、bidder 换目标），但
    结果就是「组合键数 ≈ 事件量的 29%」。
- **用户直觉「并非每人对每场拍卖出价」对真实世界成立，但 NEXMark 的 bid 引用
  模型（滑动窗口随机）刻意产生大量组合**——真实世界的拍卖会结束、bidder 会
  换目标，键数应收敛；NEXMark 的 in-flight 窗口（100 auction）让组合持续翻新。
- **结论**：q18 内存只能靠引擎机制（状态 spill，§10.5 方向 A）解决，不能靠改
  语义或数据。
